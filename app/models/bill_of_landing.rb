class BillOfLading < ApplicationRecord
  ...
  # Container count validations - ensure non-negative integers
  ...
  # Scopes
  scope :active, -> { where(valide: true) }
  scope :not_exempt, -> { where(exempte: false) }
  ...
  def blocked_for_invoicing?
    exempte? || blocked_for_refund?
  end

  # Class methods for demurrage queries
